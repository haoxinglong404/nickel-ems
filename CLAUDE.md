...
1. **「当个事儿办」缓办事项模块 + 账号实名**（07-27）：检修页改三视图——标题旁 📋记录/📊看板/🗒️当个事儿办 三个独立按钮（`setMlView` 直选高亮）；新集合 `todos`（可选关联设备，待办→已做好准备→已办，「待办」tab 含 ready；「不办了」=软删可恢复；仅 admin 可写，**rules 已发布**）；已办/不办了确认走应用内弹层（原生 prompt/confirm 在内置浏览器面板被拦的教训）；设备选择 sheet 按 `eqPickTarget` 路由复用。同日云端账号实名：总管理员→郝行龙、翔总→李宁翔（VIP 欢迎卡文案未动）；随后按用户指令**历史快照同步刷新**——检修记录 createdByName 10 条 + 二级库历史 operatorName 16 条 全部 总管理员→郝行龙（云端 0 残留）；**唐仕彬（6725051412）admin→检修班长**，现 admin 仅郝行龙一人（检修/二级库登记只有 admin 能做）。
