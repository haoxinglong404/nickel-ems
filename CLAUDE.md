...
**最近做的改动**（按时间倒序）：
1. **201 原矿浓密机给料槽搅拌 B/C 机架轴承登记**（2026-07-16 当天，张承钢；**待 push**=SEED 状态同步）：晨曦-B/C（201-PE-AG-001B/C，用户口称 2#/3#=现场编号）机架上轴承各 10 管≈4950g、机架下轴承各 6 管≈2970g，Mobilith SHC 220（同 7-14 陆壮那批——至此 6 台给料槽搅拌机架轴承全部打完），下次 12-23（160天）。**「一管」口径用户修订为 ≈550cc≈495g**（原 600cc≈540g，旧账不追改，memory 已更新）。SEED 里这 4 点还是"待加脂"+负占位时间戳的老值，一并修正。4 笔 lubehistory 时间戳按对账排序惯例（序号小/上轴承给更大时间戳，历史页从上往下 B上→B下→C上→C下）。
1. **218 回用水泵/罗茨风机/反洗泵 9 台全部 27 点登记首次润滑**（2026-07-16 补录，实际润滑 7-15 10:00~10:52 每点错开 2 分钟——**时间戳按 7-14 对账排序惯例**：历史页从上往下 = 罗茨A/B→反洗A/B/C→回用A/B/C/D、每台 轴承室→电机前→电机后，操作人张承钢=非注册按名字记 operatorId 空；**待 push**=仅 SEED 状态同步）：①轴承室 9 点——反洗泵 lp_v8_034/036/038（MOBILUX EP 2·**量未记录=标准量待定**，note 已注明）+ 回用水泵 lp_v8_040/042/044/046（MOBILUX EP 3·50g）+ 罗茨风机 lp_v8_098/100（MOBILGEAR EP 220·20L）；②用户确认"所有润滑点都加了"后补电机前/后轴承 18 点（lp_v8_033~045 奇数 + lp_v16_135~141/154/155，MOBILUX EP 2 按标准量 26.25/71.5/50.525g）。27 笔 lubehistory（`lh_178408xxxx_*`）+ 点位全部已润滑：轴承室反洗/回用 nextLubeAt=8-14（30天）、其余=10-13（90天）。**SEED 顺手修正**：这 18 个电机点 SEED status 还是旧值"待补充"（云端早已未初始），一并改"已润滑"。**发现存量口径**：`getLubeStatus` 即将阈值固定 30 天 → 周期 30 天的点刚润滑完立即显示"即将·剩29天"（旧有逻辑，待用户定夺要不要按周期比例改）。
