...
- `lubepoints` · 润滑点（**880** 个 · id = `lp_XXXX` / `lp_v6_001` / … / `lp_v13_NNN`(拆上下轴承) / `lp_v14_NNN`(污水泵结构统一) / `lp_v15_NNN`(浓密机减速机拆8点) / `lp_v16_NNN`(电机拆前/后轴承) / `lp_v17_NNN`(201B给料槽搅拌补齐轴承) / `lp_v18_001~012`(高压釜给料泵ABB电机前/后轴承·2026-07-16) / `lp_v19_001~002`(液下渣浆泵拆下轴承·2026-07-16)；2026-07-16 删 PAC/PAM 搅拌器「机架轴承」5 点=现场核实无此结构）
- `lubehistory` · 润滑执行记录
