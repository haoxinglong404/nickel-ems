...
当前版本：**v0.12**（v0.11 → 巡检模块升级为数值点检：按纸质点检表填实测数值 + 月表存储 + 趋势分析 + 导出）
...
**最近做的改动**（按时间倒序）：
1. **检修模块加"重点设备档案"（高压釜给料泵/搅拌器）+ 零件更换时间线**（2026-07-05）：检修列表"全部等级"筛选条下加**重点设备条**（⭐两组，`ML_KEY_EQ_GROUPS` 按设备名匹配：给料泵 `name==='高压釜给料泵'` 6 台 / 搅拌器 `/^高压釜\d隔室搅拌$/` 21 台；每组带 `parts` 重点零件关键词配置）。**三级整页结构**（用户明确不要半屏 sheet）：检修列表 → `maintenance-keyeq`（组内设备卡=位号+ABC徽章+记录数+最近日期）→ `maintenance-eqlog`（单台检修档案：`全部记录` tab + 零件 tab）。**零件更换时间线** `renderMlPartTimeline`：记录按关键词从 faultParts/spareParts/faultCause/measures 模糊归类（给料泵：单向阀[单向阀,止逆]/隔膜/隔热组件[隔热]；搅拌器：机械密封[机封]/减速机/搅拌桨[桨叶]——搅拌器三个是我暂定的，用户未确认），节点=更换日期+「用了 N 天（约X周/月）后更换」（最新一条=「在用·已 N 天」）+该次备件品牌规格芯片+原因摘要，顶部显平均更换间隔；**使用时长对比条形图** `renderMlPartLifeChart`（条长∝天数、颜色=备件名、图例、在用=半透明+标注）——给采购对比供应商寿命用。**返回链路**：`openMlDetail(id, from)` 加第二参 + `mlDetailFrom`，从档案页进详情返回回档案页；`backToMlKeyEq` 按当前设备 `mlEqlogGroup()` 归组返回。switchModule 两个新 module（keyeq/eqlog）都隐藏顶栏底栏、导航高亮"检修"。`renderMlCard(ml, from)` 加参（⚠️ `renderMaintenanceList` 已改 `.map(m=>renderMlCard(m))`，不能直接 `.map(renderMlCard)` 否则 map 的 index 会被当 from）。**预览演示数据** `previewMlDemoRecords()`：仅 ?preview=1 时 `subscribeMaintenance` 快照后往 mlCache concat 9 条 106A 演示记录（`ml_demo_*`，GEHO vs 国产件寿命对比剧情，备注标"预览演示数据"，不写云端；代码保留有 PREVIEW_MODE 保护）。上一版做过又被替换的"主列表只看一台"过滤已删干净。
...
- 🚀 **数值点检 v2 上线三步（2026-07-04 改完待执行）**：① 用户先在预览验收 → ② Console 发布 firestore.rules（加 `inspectmonthly`）→ ③ push 代码（线上自动跑模板迁移）→ 真实登录提交一台点检 + 试导出验证
