...
**最近做的改动**（按时间倒序）：
1. **中和车间「微油螺杆空压机」点检模板油温限值 45℃ → 95℃**（2026-08-18，代码 1 行 + 云端 1 个模板文档）：用户发来 `109-215-PE-CM-003A`《微油螺杆压缩机日常点检记录表》照片，红笔圈出「油水分离系统 → 油温」一列——纸质表印的限值是 **温度＜45℃**，而四行实测值是 **76.4 / 78.0 / 68.6 / 71.5℃**，条条超限。用户指令："他这个类型的空压机油温给他改成小于95度"。

   **定位**：全库只有一个「微油螺杆」模板 —— `zh_compressor_oil`（名称「空压机·微油螺杆」，中和 14 个 `zh_` 模板之一，2026-08-08 那批建的）。另外两个空压机模板 `zh_compressor_oilfree`（无油螺杆）/ `zh_compressor_centrifugal`（离心式）**没碰**；酸浸的 `compressor`/`compressor_pump`（油温＜35℃）也没碰。用 `where('inspectTplKey','==','zh_compressor_oil')` 定向查询确认**受影响设备正好 2 台**：`eq_zh_199` = `109-215-PE-CM-003A`、`eq_zh_200` = `109-215-PE-CM-003B`，均 `plant:'zh'`、`noInspect:false`。

   **改动内容**（只改这一个测点）：`{g:'油水分离系统'}` 里的 `_num('油温','℃','＜45℃','测温枪',45)` → `_num('油温','℃','＜95℃','测温枪',95)`，即 `max: 45→95`、`std: '＜45℃'→'＜95℃'`。**冷却系统那一组照纸质表原样保留**（进水＜45 / 回水＜55 / 进油＜65 / 回油＜75），压缩机组的排气温度＜65、排气压力＜1.0MPa 也没动 —— 用户红笔只圈了油水分离系统那一处。

   **为什么必须同时写云端**：`getInspectTemplateForEquipment()` 的逻辑是「云端优先，只有云端 `rev` 与 SEED `rev` 不一致（或 `v` 不一致）时才回退 SEED」。云端该文档 `rev:2`、SEED `INSPECT_TPL_REV=2`，**两边相等 → 云端赢**，所以只改 index.html 种子线上根本不生效。两条一次性迁移（`applyInspectTplV2IfNeeded` / `seedInspectionTemplatesIfEmpty`）都是 marker 幂等且早已跑过，不会把新种子推上云。做法沿用 2026-08-08 建 `zh_` 模板那批的路子：**preview 会话（localhost `?preview=1`）里 import firebase 12.12.0 直接 `updateDoc` 云端文档，`rev` 保持 2、不新增迁移 marker**（种子与云端内容改成完全一致，rev 相等即"版本一致"）。写入脚本先深拷贝 `groups`、只对 `g==='油水分离系统'` 且 `n==='油温' && t==='num'` 的测点改值，**命中数 `hit!==1` 直接 ABORT**，写完立刻回读核验（`std:'＜95℃'`、`max:95`，冷却系统仍为 45/55/65/75）。

   **超限记录清理 → 无可清**：用户追加"超限这个也清一下嘛"。测点 key 按 `gr.g + '·' + (pt.p ? pt.p+'·' : '') + m.n` 生成 = **`油水分离系统·油温`**。用 `where('equipmentId','in',['eq_zh_199','eq_zh_200'])` 查 `inspectmonthly` 与 `inspecthistory`：**整个 `inspectmonthly` 只有 1 个文档 `im_eq_zh_199_202608`，里面只有 1 天（8-17，代龙权 `6726042268` 提交），且 `stopped:true`（设备停机未运行）→ `v:{}`、`abn:[]`、`notes:{}` 全空；`inspecthistory` 一条都没有**（v2 设备只在有异常时才写这条，从没触发过）。也就是说**纸质表上那 4 行数值从来没有录进 EMS**，系统里不存在任何 `油水分离系统·油温` 的超限标记，无需清理也无从清理。日后若把纸质表数值补录进系统，用的已是新限值 95℃，不会再判超限。

   **提交注意**：commit 时工作区里还带着**另一个会话未完成的 `noReplace`（检修"未更换新件"）改动**（约 40 行，涉及 `renderMlPartTimeline`/`mlbUnitStats`/`renderMlCard`/`renderMaintenanceDetail`/`openMlCreate`/`renderMlForm` 与两条 CSS）。为不把别人未验证的功能捎带上线，走了**只暂存单行**的路子：`git show HEAD:index.html` 取出基线 → 单独打上 45→95 这一处 → `git hash-object -w` + `git update-index --cacheinfo` 直接把这个 blob 塞进索引 → `git commit`（不加 `-a`）。**全程没动工作区文件**，`noReplace` 那份改动原样留在工作区继续未提交。

   **验证**：esprima `parseModule` 通过（`??`→`||` 后）；preview 页面加载 0 报错；云端回读核验通过；`git diff --cached` 确认暂存区只有 1 行改动。
1. **2系列高压釜给料泵曲轴箱换油补录 2 笔**（2026-08-18 登记，实际润滑 2026-08-04，纯云端数据·无代码变更）：用户口述"2系列两台高压釜给料泵更换曲轴箱润滑油，8 月 4 号，润滑人张承钢"。

   **目标点位**（云端查得，两个点此前**一笔历史都没有**、状态皆为「未初始润滑」，不存在重复登记）：
   - `lp_0099` · `203-PE-PP-206A` · 2系列-高压釜给料泵-A泵 · 曲轴箱
   - `lp_0103` · `203-PE-PP-206B` · 2系列-高压釜给料泵-B泵 · 曲轴箱
   - 两点同规格：标准油 SP320（`mobilOil` MOBILGEAR 600 XP 320）、标准量 1400 L、周期 330 天 / 8000 h。

   **口径**（用户当场拍板）：①**用量按标准量 1400 L** 各记一笔（无纸质表实测值）；②**实际油品 = Omala S2 GX 320**（壳牌，与台账标准油 SP320 同为 ISO VG320 矿物齿轮油，等效替代）；③备注写 **「首保更换」**；④操作人张承钢是**注册用户**（工号 `6725031231`，角色 maintenance-leader），`operatorId` 按工号记（不同于何甫林/王宗立那种按名字记的非注册用户）；⑤`lubeAt` = 2026-08-04 **10:00**（沿用历次补录口径），`createdAt/updatedAt` = 真实录入时刻。

   **写入**：preview 会话（localhost `?preview=1`，浏览器已带匿名 auth token）里 `import` firebase 12.12.0 模块直接 `writeBatch` 原子提交 —— 2 笔 `lubehistory`（`lh_1785808800000_0_yfb` / `lh_1785808800000_1_y35`，id 格式 `lh_<lubeAt>_<i>_<rand>`，字段照 `confirmBulkLube` 逐条对齐 + 手动 `plant:'hpal'`）+ 2 个 `lubepoints` 更新（`lastLubeAt`=lubeAt、`nextLubeAt`=lubeAt+330天=**2027-06-30**、`status`='已润滑'）。回读核验全部通过。

   **踩坑**：页面用的是 **firebase 12.12.0**，控制台里 `import` 10.7.1 会拿到**另一套模块注册表**，`getApps()` 返回空数组（表现为 "NO APP"）—— 直接写云端时**必须 import 与 index.html 同版本的 SDK URL**。

   **收尾**：08-18 用户在线上确认两笔记录显示正常；台账标准油号 SP320 **拍板不改**（现场实际用 Omala S2 GX 320 只体现在这两笔记录的 `actualOil` 里）。
1. **非酸浸车间只保留「设备台账 + 点检」**（2026-08-17，纯代码 3 处·无云端/rules/迁移变更）：用户要求"其他几个车间的重点设备看板、检修、润滑等模块都删了"。**不是删代码**（酸浸还在用），是按当前车间隐藏。
   ...
   **④ 同日跟进：中和/产品润滑数据整体删除（纯云端·2131 条）**
   - 用户："把除了酸浸车间的其他的数据删了吧，其他车间真不用"。**先澄清了两件事再动手**：(a) **删云端数据不会让 index.html 变小** —— 文件里 615 KB 种子数据全是酸浸的（`lp_zh_`/`lp_cp_`/`eq_zh_`/`eq_cp_`/`eq_zs_` 在文件里 0 命中），非酸浸车间的数据当初就是直接写进云端、从没进过代码；(b) **这三个车间的设备台账和点检正在用**（点检模块就是靠 equipments 跑的，产品 08-11 刚做了一批点检），不能一起删。
   - 盘点后确认：真正"因隐藏模块而变成废数据"的只有润滑 —— `maintenancelogs`/`todos`/`secondstock`/`secondstockhistory` 这三个车间**本来就是 0 条**。用户选择「只删润滑数据」。
   - **删前完整备份**：抓 927 个点 + 1204 笔历史 → 桌面 `中和产品润滑数据备份_2026-08-17.json`（877 KB，全字段、id 齐全，可原样写回）。走 memory 里那条「超大结果自动落盘 + Python 解码」的路子；**踩坑**：`btoa(String.fromCharCode(...bytes))` 在 90 万字节上爆栈（Maximum call stack size exceeded），改成按 8192 字节分块拼接；另外落盘文件尾部带工具的 Tab Context 说明，直接 b64decode 会因非 ASCII 报错，要用 `re.findall(r'[A-Za-z0-9+/=]{5000,}')` 取最长串。
   - **删除**：复用页面上已抓到的那批 id（`window.__bk`），**不再重复读云端**；三道防呆（非 zh/cp 立即中止 / 条数必须等于盘点值 927+1204 / id 必须齐全），writeBatch 每批 400 共 6 批。⚠️ **工具 30 秒超时但页面里的删除仍在继续**，靠 `window.__bkDeleted` 标记回查确认完成 2131 条 —— 长批量操作超时不等于失败，先查标记再判断。
   - **结果**：`lubepoints` 1820→**893**、`lubehistory` 1857→**653**（全部只剩酸浸，与 CLAUDE.md 里的酸浸数逐条吻合）；`equipments` 845、`inspectmonthly` 580 一条没动。
...
1. **8月润滑计划（第3周）补录 36 笔 + 1系列1隔室搅拌补建 2 个电机脂点**（2026-08-17，云端数据 + 1 处 SEED 同步）：用户给两张纸质计划表照片（36 行全部有执行记录，无未执行行），OCR 后出核对表由用户逐项订正，再走 preview 会话 writeBatch 一次原子写入。
   ...
   **① 表的内容与订正**
   - 36 行 = 高预给料泵电机脂点 12（8-10 执行）+ 高压釜 2~7 隔室搅拌电机脂点 12（8-11）+ 高压釜给料泵 106A/B 电机脂点 4（8-12）+ 褐铁矿浓密机 1#~8# 减速机轴承 8（8-14）。
   - **用户订正 3 处**：(a) 序 5（104A 电机前）实际量是 72g 不是我读的 50g；(b) 两台给料泵**都是 8-12** 执行（我原读 8-11/8-12 混排）；(c) 浓密机 8 行**全部 8-14**（"给料泵之后就是 8-14"）。
   - **操作人**：除高压釜给料泵 4 行是**何甫林**外，其余 32 行都是**王宗立**。两人在 `users` 里都查不到 → 全部 `operatorId:''` 按名字记（与第1/2周何甫林口径一致）。
   - 实际量：只有给料泵 4 行超规定（前轴承 90→95g、后轴承 75→80g），其余等于规定量。合计 9874g。

   **② 照片里读不出来的一处，靠系统数据反查确定**
   - 最后 8 行拍出来是「#减速机油箱1000」，前面的编号糊掉了。查云端 `202-PE-TH-101` 正好有 **1#~8# 减速机轴承**（`lp_0259` + `lp_v15_001~007`），标准量恰好 1000g、2#极压锂基脂 ↔ 表上 Mobil Lux EP 2 —— 八行八个点，一一对应，不是"油箱"。

   **③ 1系列1隔室搅拌（`203-PE-AG-101A`）的台账缺口（用户问出来的）**
   - 用户注意到计划表从"2隔室"起、没有1隔室，让我查点位。结果：**21 台高压釜隔室搅拌里只有 101A 是 2 个点**（减速机 + 机械密封轴承），其余 20 台（101B~G / 201A~G / 301A~G）都是 4 个点，缺的正是电机前/后轴承。
   - **铁证是 id 缺号**：这批点位每台两个连号（电机前, 机封）—— 101B=`lp_0338`+`lp_0339`、201A=`lp_0340`+`lp_0341`；轮到 101A 只有 `lp_0336`（机封），**`lp_0337` 整个空缺**。即建库时电机前轴承点就漏建了；2026-04 那批 `lp_v16_*` 拆前/后轴承是照已有的前轴承点拆的，101A 没有前轴承点 → 后轴承也跟着没有。
   - **这解释了纸质表为什么排不出 1 隔室搅拌**：五定表是照系统台账出的，系统里没这两个点。不是工人漏打。
   - 用户拍板补建：新增 `lp_v22_001`（电机前轴承）/ `lp_v22_002`（电机后轴承），照 101B~G 同规格 —— 51g、3#锂基脂、`mobilOil` MOBILUX EP 3、125 天/3000h、`plant:'hpal'`、状态「未初始润滑」，**本周不补登记录**（本来就没打）。下次出五定表会自动排上。
   - 顺带发现：101A/101B 减速机加注量记 150L、101C~G 记 130L —— 归入老待办「台账 150/130 vs 铭牌≈119L」，本次未动。

   **④ 写入与核验**
   - 脚本两道防呆：先按 `note` 查重（已存在同备注记录即中止，可安全重跑）；再 `where('id','in',...)` 分块确认 36 个点位都在（缺一个即中止）。74 次写入（36 历史 + 36 点位更新 + 2 新点）一个 `writeBatch` 原子提交。
   - 回读核验全过：36 笔、日期分布 12/12/4/8、王宗立 32 / 何甫林 4、油品 EP2 20 / EP3 12 / SHC220 4、合计 9874g 与手算逐项吻合；点位状态转「已润滑」且 `nextLubeAt` 按各自周期顺延（高预 125 天→12/13、浓密机 80 天→11/2）；101A 现 4 个点。
   - 集合总数：`lubehistory` 1820→**1857**（+36 本批 +1 郝行龙 8-12 那笔 206A 联轴器）、`lubepoints` 1818→**1820**。
   - **口径沿用第1/2周**：`lubeAt` = 实际执行日 10:00（浏览器 UTC+8 = 印尼 WITA）、`note`「第3周润滑计划（8.10-8.14）补录」、`actualOil` 写计划表牌号（美孚锂基脂 Mobil Lux EP 2 / EP 3 / 美孚 Mobilith SHC 220）、`plant:'hpal'`。
   - 本地 SEED 只同步了 2 个新点（历史记录按惯例不进 SEED）；`esprima.parseModule` 解析通过 + preview 加载 0 报错，文件 1.16 MB。

   **⑤ 08-17 晚补记 2 笔（101A 电机）**：用户回头确认"1 系列 1# 高压釜搅拌器的电机是润滑了的"（当时存疑所以没记）。按与 2~7 隔室同批的口径补录 —— `lubeAt` 8-11 10:00、51g、美孚锂基脂 Mobil Lux EP 3、王宗立，`note` 在原文后追加"（纸质表漏印此两行，08-17 现场确认已执行）"，`lp_v22_001/002` 由「未初始润滑」转「已润滑」、下次到期 2026-12-14。校验：8-11 当天 `203-PE-AG-101A~G` 七台各 2 笔齐全，`lubehistory` 653→**655**。脚本防呆：这两个点已有记录则中止（防重复执行）+ 点位 tag 必须是 101A。

   **⑥ 顺带观察**：`users` 现有 **48 个账号**（CLAUDE.md 记的是 08-12 核实的 43 个），像是有人新建了账号，未逐一核名单 —— 若是制酸车间建的，「制酸 0 账号」那条待办可销。

1. **v0.26 重点设备看板按领导手绘改版：方框网格 + 预热器泵进重点设备 + 零件消耗表**（2026-08-15，纯代码·无云端/rules/迁移变更）：领导给了一张手绘草图，看板从"图表堆"改成"设备方框墙"。同会话内按用户反馈迭代了五轮。

   **① 布局改版（草图主体）**
   - 顶部 1/2/3 系列胶囊不变；下面改为**设备方框网格**（`mlbEqGridHtml`）：高压釜给料泵 2 台（A/B）、高压釜搅拌器 7 台（1~7 隔室）、**预热器给料泵 10 台**（新进重点设备）。
   - 方框上直接显示状态：`⚠ 单向阀 超寿命`（橙框）/ `累计 N 次` / `暂无记录`；点方框**就地展开零件卡**（`mlbEqExpandHtml`，展开状态 `mlBoardOpenTag`，切系列重置、从档案页返回保持）。
   - 展开卡每零件一行：**上次更换日期 / 在用天数 / 平均寿命 / 共 N 次 + 「在用：备件名」**；点零件行 → `mlbOpenEqlog(tag, pkey)` 进单台档案页并直接选中该零件的更换时间线（= 草图底下那个"维修记录表"，复用现有页面零新码）；卡底「查看完整档案」进全部记录。
   - 网格列数：给料泵 2 列 / 搅拌器 4 列 / 预热器泵 3 列，宽屏（≥1024px）分别放宽到 4/7/5 列。

   **② 预热器给料泵进重点设备**（监控机械密封）
   - `ML_KEY_EQ_GROUPS` 加第三组 `preheater`，match `^[123]系列-(中预|高预)[一二三]级泵`（自动排除密封液系统/密封液给料泵/补液站），唯一零件 = 机械密封（tags `机械密封`，kw `机械密封/机封`）。
   - 每系列 10 台 = 中预一/二级 + 高预一/二/三级 × A/B 两台；**warman / 五二五 两厂家分开两片方框**（用户要求），方框名去厂家后缀（「中预一级」），片头标厂家。
   - `mlbShortName` 加分支：`中预一级·warman` 这样的短名。检修列表页重点设备条自动多出「⭐ 预热器给料泵」入口（`renderMlKeyEqPage` 通用，零改动）。
   - **老记录自动归类生效**：这些泵已有的 21 条检修记录里，登记时勾过「机械密封」标签的机封更换直接进了看板，不用补录。

   **③ 原统计图表撤掉，换零件消耗表**（用户第五轮要求）
   - 删掉：概览卡（累计更换/预警数）、超寿命预警卡列表、更换次数占比环形图、各台设备条形图、近 6 月趋势 —— `mlbDonutSvg` / `mlbSeriesBlockHtml` / `mlbTrendHtml` / `mlbGroupRank` 四个函数整体删除（超寿命信息不丢，方框 ⚠ 就是）。
   - 新增 `mlbConsumeCardHtml`：每种重点零件一行 —— **本月消耗 / 本季度消耗（自然月/自然季度）/ 平均使用寿命**；没记录的零件行隐藏。
   - **厂家/规格寿命对比卡保留**并泛化：`mlbCheckvalveBrandStats`/`mlbCheckvalveCardHtml` → `mlbBrandStats`/`mlbBrandCardHtml(stats, gkey, pkey, title)` 通用函数，现调两次（单向阀 + 预热器泵机封）；**从"三系列合计"改为跟随当前系列**（用户最后一轮要求）。

   **④ 「主备件」统一口径**（用户看到"O型圈混进在用"、"垫片冒充机封厂家"后逐步收紧）
   - `mlbPartMainSpare(m, g, pkey)`：kw 归属命中的备件里，按零件 kw 顺序挑名称先命中的一条（「单向阀组件」优先于按「阀盖」kw 归进来的 O 形圈）；没命中且该次只修这一个重点零件时兜底取全部备件的第一条。
   - 展开卡「在用」= 该主件的 `mlSpName`（二级库现名，含厂家规格，如「单向阀组件 （达索）260131」）。
   - **消耗表数量和对比卡厂家名只认名字确实带零件关键词的主备件**：该次只登记了垫片/O形圈这类附件 → 消耗不计数量、对比卡归「未登记厂家/规格」（垫片名不再单独成行）。顺带发现二级库种子数据有「个Piece」这类双语单位原样，属数据 quirk 非代码问题。
   - **提醒用户的登记规范**：机封更换尽量从二级库选名字带「机械密封」的条目、厂家写进名字（学达索），对比图才能分厂家。

   **⑤ 修双计数 bug（顺带发现，重要）**
   - 搅拌器和预热器泵的零件 pkey 都叫 `mechseal`，而看板统计循环原来对每台设备遍历**全部** `MLB_PART_DEFS` —— 同一台预热器泵的机封记录被同时算进「高压釜搅拌器机械密封」和「预热器给料泵机械密封」（预警卡重复两条、环形图虚高）。
   - 修法：所有 `MLB_PART_DEFS` × stats 的循环补 `d.gkey === s.gkey` 过滤（renderMlBoard 预警统计、图例计数、条形图、趋势图 4 处；后两处随图表删除已不存在，过滤规则记入 CLAUDE.md 防再犯）。

   **⑥ 二级库现名的实时性（用户发现"达索"不显示后修）**
   - 根因：v0.24 按需订阅后 **board 模块没订 `secondstock`** → `mlSpName` 拿不到现名退回登记快照。`MODULE_SUBS.board` 加 `secondstock`（约 300 读/会话）。
   - 时序补丁:二级库数据比检修晚到时首屏显示旧名 → `subscribeSecondStock` 回调加一根看板刷新线（board active 时 `renderMlBoard()`）。
   - 文案：展开卡「最近」→「上次更换」。
   - 验证：esprima 语法解析、window 函数暴露检查、preview 真实云端数据全链路（方框状态/展开/零件行跳转/返回保持/系列切换/三张卡口径）逐轮回归，控制台 0 报错。
1. **登录两处收尾修复：密码框未展开误报「密码错误」+ 长轮询标记自动过期**（2026-08-12，纯代码 2 处·无云端/rules/迁移变更）：用户问「刚才的改动会带来什么坏处」，逐条自查后拍板修这两处。

   **① 认证慢 → 密码框不展开 → 误报「密码错误」**（第一刀引入的副作用）
   - 第一刀给工号输入框加了**防抖 500ms + `authUser` 就绪才查云端**（省额度），代价是**认证慢时密码框不会展开**。
   - 用户没地方输密码，直接点「进入系统」→ `pw` 是空串 → 撞上 `if (user.password && pw !== user.password …)` → 报**「密码错误」**。人会以为自己记错了密码，实际是没地方输。
   - 改法：在密码比对**之前**插一个空密码分支 —— `applyLoginFields(user)` 把密码框展开 + 提示「该账号需要密码，请输入密码后再试」+ `focus()` 到密码框，不再走「密码错误」。
   - 注：iOS 上 `focus()` 因为跨了 await 已经脱离用户手势上下文，键盘不一定弹出来，但输入框会聚焦，不影响使用。

   **② 长轮询降级标记没有解除路径**（第三刀的设计缺口，我自己在备忘录里记过）
   - 原来标记值是 `'1'`，只在「登录连续两次超时」时置位，**没有任何解除机制**。而车间信号差时，纯粹的网络抖动完全可能触发 → 一旦置位就**永久走长轮询**（流量更高、延迟略增）。
   - 改法：标记值改存 **`Date.now()` 时间戳**，新增 `NET_DEGRADE_TTL_MS = 3 天`。启动时读到标记：未过期 → 继续走长轮询；**过期 → 删标记，回到默认流式连接再试一次**（真有问题会当场重新置位，形成闭环）。`markNetDegraded()` 也改写时间戳。
   - **老格式向后兼容**：读到 `'1'`（或 parse 不出数字）仍按降级处理，同时补上当前时间戳，从那一刻起算 3 天 —— 不会因为格式变更让已置位的客户端突然失效。
   - 3 天这个值：短到临时抖动能自己好，长到真有网络问题的车间不会天天卡一次。

   **③ 一并说明的其余取舍**（用户问了，逐条答过，决定不动）：IndexedDB 探测可能因设备卡顿误判（有自愈路径）；探测留一个空库 `ems-idb-probe`（占用可忽略）；密码框展开晚 0.5 秒；按钮改 `type="button"` 后不再触发表单提交，**Safari 可能不再主动弹「保存密码」**（只影响新账号首次登录）；登录态从 IndexedDB 挪到 localStorage **安全性无差异**（同源都可读，且是匿名 token）。

   **④ 验证**：登录桩测 30 → **36 项断言**（新增【M】组 6 项：前提是密码框收起 / 提示文案正确 / 不再报密码错误 / 自动展开 / 未放行 / 补输密码后正常登录），IndexedDB 桩测 12 → **20 项断言**（新增【7】【8】【9】：默认不开长轮询、1 天前置位仍生效、4 天前置位自动过期并清标记、老格式 `'1'` 兼容并补时间戳、`markNetDegraded` 写入时间戳）。全过。过程中桩环境补了 `focus()`/`style` —— 真实 input 一定有 `focus`，是桩的缺口不是产品缺陷。
